...
class Car:
    ...
    def turn(self, direction):
        if direction.lower() not in {"left, right"}:
            raise ValueError("Введите корректное значение для поворота.")
        elif direction == "left":
...
class AbstractVillageCar(Car):
    ...
    def show_speed(self):
        ...
        if speed_info["speed"] > self.speed_limit:
            speed_info["messages"].append("Вы превышаете скорость!")
        return speed_info
...
car = WorkCar(speed=150, color="red", name="ford", is_police=False)
car.turn("right")
