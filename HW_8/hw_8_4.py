...
class Scanner(Technics):
    def scan_smth(self, data):
        return f"Scanner {self.model} s/nm {self.serial_no} scanned {data}"
...
class Warehouse:
    ...
    def get_several_tecs_to_warehouse(self, num: int, tech_list):
        if type(num) != int:
            raise ValueError("Введите число! Невалидноче количество техники!")
    ...
    def get_tech_to_warehouse(self, technics: Technics):
        if self.total == self.max_volume:
            raise OverflowError("Склад полный!")
